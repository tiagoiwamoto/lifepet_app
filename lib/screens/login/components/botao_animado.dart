...
class BotaoAnimado extends StatelessWidget {
  ...
  BotaoAnimado({this.controller}) :
      ...
      ).animate(CurvedAnimation(parent: controller, curve: Interval(0.0, 0.150))),
      crescerBotao = Tween(
        begin: 45.0,
        end: 1000.0
  ...
  Widget _construirAnimacao(BuildContext context, Widget child){
    return InkWell(
      ...
      child: crescerBotao.value <= 45 ? Container(
        height: 45,
        ...
        decoration: BoxDecoration(
          color: Colors.redAccent,
          shape: crescerBotao.value < 500 ? BoxShape.circle : BoxShape.rectangle
        ),
      ),
    );
  }
